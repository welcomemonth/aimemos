...
// ==========================================
// 3. ConnectRPC 实现 (用于 Web 前端) - **修复报错的关键**
// ==========================================
func (s *ConnectServiceHandler) Chat(
	ctx context.Context,
	req *connect.Request[v1pb.ChatRequest],
	stream *connect.ServerStream[v1pb.ChatResponse],
) error {
	// 调用上面抽取的公共逻辑
	return s.APIV1Service.streamChatWithOpenAI(ctx, req.Msg.Message, func(content string) error {
